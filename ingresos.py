...
class ingresos:
    def __init__(self):
        ...
        #labels
        #registrar ingresos
        self.registrarIngresos=tkinter.Label(self.ventana,text="Registrar Ingresos",font=("arial",15,"bold"),bg="#8085AC")
        self.registrarIngresos.place(x=15,y=15)
        self.ingreso=tkinter.Label(self.ventana,text="Ingreso ($)",font=("arial",13,"bold"),bg="#8085AC")
        self.ingreso.place(x=15,y=45)
        ...
        self.filtroIngreso=tkinter.Label(self.ventana,text="Ingreso ($)",font=("arial",13,"bold"),bg="#8085AC")
        self.filtroIngreso.place(x=165,y=150)
        ...
        #tabla
        self.ingresosAlmacenados=tkinter.Label(self.ventana,text="Ingresos Almacenados",font=("arial",15,"bold"),bg="#8085AC")
        self.ingresosAlmacenados.place(x=15,y=358)

        #entrys
        #registrar ingresos
        self.entryIngreso=tkinter.Entry(self.ventana,font=("arial",13,"bold"),width=9)
        self.entryIngreso.place(x=15,y=70)
        self.entryOrigen=tkinter.Entry(self.ventana,font=("arial",13,"bold"),width=9)
        self.entryOrigen.place(x=165,y=70)
            #lista deplegable
        self.listaDesplegable=tkinter.ttk.Combobox(self.ventana,values=["Laboral","Inversion","Prestamo","Otro"],state='readonly',font=("arial",13,"bold"),width=9)
        ...
        self.entryfiltroIngreso=tkinter.Entry(self.ventana,font=("arial",13,"bold"),width=9)
        self.entryfiltroIngreso.place(x=165,y=175)
        ...
        self.entryfiltroMes.place(x=165,y=230)
            #lista deplegable
        self.filtrolistaDesplegable=tkinter.ttk.Combobox(self.ventana,values=["Laboral","Inversion","Prestamo","Otro"],state='readonly',font=("arial",13,"bold"),width=9)
        ...
        self.botonfiltroIngreso=tkinter.Button(self.ventana,text="Filtrar",font=("arial",7,"bold"),width=5,height=1,command=self.funcionfiltroIngreso)
        self.botonfiltroIngreso.place(x=255,y=178)
        self.botonfiltroIngreso.config(cursor="hand2",bg="#9C9594",activebackground="#9C9594",bd=2,borderwidth=3)
        ...
        self.botonfiltroCategoria.place(x=425,y=233)
        ...
        #tabla treeview
        self.tabla=tkinter.ttk.Treeview(self.ventana,columns=('Registro','Ingreso ($)','Origen','Categoria','Fecha de Registro'),yscrollcommand=self.scrollVertical.set)
        self.tabla.place(x=15,y=385)
        self.tabla.config(height=8)
            #encabezados
        self.tabla.heading('#1',text='Registro')
        self.tabla.heading('#2',text='Ingreso ($)')
    ...
    def info(self):
        tkinter.messagebox.showinfo("INFORMACION","Esta interfaz de la aplicacion tiene la capacidad de registrar tus ingresos,"
                                    " eliminar registros y filtrar segun tus consideraciones. Ademas, posee una tabla "
                                    "que permitira monitorear tus ingresos")
    def acercade(self):
    ...
    def guardar(self,ingreso,origen,categoria,fecha):
        self.conexion=sqlite3.connect('datos.db')
        self.cursor=self.conexion.cursor()
        self.cursor.execute("INSERT INTO ingresos(Ingreso,Origen,Categoria,Fecha) VALUES(?,?,?,?)",(ingreso,origen,categoria,fecha))
    ...
    def cargar(self):
        ...
        self.conexion=sqlite3.connect('datos.db')
        self.cursor=self.conexion.cursor()
        self.cursor.execute("SELECT * FROM ingresos")
    ...
    #funciones de botones
        #registrar
    def registrar(self):
        self.variableIngreso=self.entryIngreso.get().strip()
        self.variableOrigen=self.entryOrigen.get().strip()
        self.variableCategoria=self.listaDesplegable.get()
        if not self.variableIngreso or not self.variableOrigen or not self.variableCategoria:
            tkinter.messagebox.showinfo("AVISO","Los campos Ingreso y Origen tienen que estar llenos, al igual que seleccionar un tipo de Categoria para poder registrar la informacion")
        else:
            try:
                self.variableIngreso=float(self.variableIngreso)
                if self.variableIngreso < 0:
                    tkinter.messagebox.showerror("ERROR","El campo Ingreso solo permite valores numericos mayores o iguales a 0")
                else:
                    self.variableFecha=date.today().isoformat()
                    self.guardar(self.variableIngreso,self.variableOrigen,self.variableCategoria,self.variableFecha)
                    self.cargar()
            except:
                tkinter.messagebox.showerror("ERROR","El campo Ingreso solo permite valores numericos mayores o iguales a 0")
        #filtros
        ...
        if not self.variablefiltroRegistro:
            ...
            try:
                ...
                if self.variablefiltroRegistro <=0:
                    ...
                    self.conexion=sqlite3.connect('datos.db')
                    self.cursor=self.conexion.cursor()
                    self.cursor.execute("SELECT * FROM ingresos WHERE Registro=? ORDER BY Fecha DESC",(self.variablefiltroRegistro,))
                    self.filas=self.cursor.fetchall()
                    self.conexion.close()
    ...
    def funcionfiltroIngreso(self):
        self.variablefiltroIngreso=self.entryfiltroIngreso.get().strip()
        if not self.variablefiltroIngreso:
            tkinter.messagebox.showinfo("AVISO","El Campo Ingreso de la seccion de filtros tiene que estar lleno para poder hacer el filtro")
        else:
            try:
                self.variablefiltroIngreso=float(self.variablefiltroIngreso)
                if self.variablefiltroIngreso < 0:
                    tkinter.messagebox.showerror("ERROR","El campo Ingreso de la seccion de filtros tiene que ser un valor numerico mayor o igual a 0")
                else:
                    for self.row in self.tabla.get_children():
                        self.tabla.delete(self.row)
                    ...
                    self.conexion=sqlite3.connect('datos.db')
                    self.cursor=self.conexion.cursor()
                    self.cursor.execute("SELECT * FROM ingresos WHERE Ingreso=? ORDER BY Fecha DESC",(self.variablefiltroIngreso,))
                    self.filas=self.cursor.fetchall()
                    self.conexion.close()
                ...
                tkinter.messagebox.showerror("ERROR","El campo Ingreso de la seccion de filtros tiene que ser un valor numerico")
    def funcionfiltroOrigen(self):
        ...
        if not self.variablefiltroOrigen:
            ...
            self.conexion=sqlite3.connect('datos.db')
            self.cursor=self.conexion.cursor()
            self.cursor.execute("SELECT * FROM ingresos WHERE Origen=? ORDER BY Fecha DESC",(self.variablefiltroOrigen,))
            self.filas=self.cursor.fetchall()
            self.conexion.close()
    ...
    def funcionfiltroFecha(self):
        ...
        if not self.variablefiltroFecha:
            ...
            self.conexion=sqlite3.connect('datos.db')
            self.cursor=self.conexion.cursor()
            self.cursor.execute("SELECT * FROM ingresos WHERE Fecha=? ORDER BY Fecha DESC",(self.variablefiltroFecha,))
            self.filas=self.cursor.fetchall()
            self.conexion.close()
        ...
        if not self.variablefiltroMes:
            ...
            try:
                ...
                if self.variablefiltroMes < 1 or self.variablefiltroMes > 12:
                    ...
                    self.conexion=sqlite3.connect('datos.db')
                    self.cursor=self.conexion.cursor()
                    self.cursor.execute("SELECT * FROM ingresos WHERE STRFTIME('%m',Fecha)=? ORDER BY Fecha DESC",(self.variablefiltroMes,))
                    self.filas=self.cursor.fetchall()
                    self.conexion.close()
    ...
    def funcionfiltroCategoria(self):
        ...
        if not self.variablefiltroCategoria:
            ...
            self.conexion=sqlite3.connect('datos.db')
            self.cursor=self.conexion.cursor()
            self.cursor.execute("SELECT * FROM ingresos WHERE Categoria=? ORDER BY Fecha DESC",(self.variablefiltroCategoria,))
            self.filas=self.cursor.fetchall()
            self.conexion.close()
    ...
    def funcionEliminarregistro(self):
        ...
        if not self.variableEliminarregistro:
            ...
            try:
                ...
                self.conexion=sqlite3.connect('datos.db')
                self.cursor=self.conexion.cursor()
                self.cursor.execute("SELECT Registro From Ingresos")
                ...
                if self.variableEliminarregistro not in self.registrosDisponibles:
                    ...
                    self.conexion=sqlite3.connect('datos.db')
                    self.cursor=self.conexion.cursor()
                    self.cursor.execute("DELETE FROM Ingresos WHERE Registro=?",(self.variableEliminarregistro,))
                    self.conexion.commit()
                    self.conexion.close()
